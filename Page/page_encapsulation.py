...
class Base(object):
    ...
    # 循环滚动列表，直到找到目标元素并触发点击事件
    def cycling_up(self, t=500):    # (driver, t=500)
        s = self.driver.find_element_by_class_name('列表容器').size
        x1 = s['width'] * 0.5
        y1 = s['height'] * 2 * 0.75
        y2 = s['height'] * 2 * 0.25
        i = 0
        while i < 10:
            try:
                self.driver.find_element_by_name('不在屏幕内的目标元素').click()
                break
            except Exception as e:
